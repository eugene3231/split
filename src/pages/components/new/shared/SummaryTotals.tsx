...
export function SummaryTotals({ split, discount, serviceCharge, gst }: Props) {
  ...
  return (
    <div className="space-y-4 pt-6 border-t border-surface-container-high mb-8">
      <div className="flex justify-between items-center text-sm">
        ...
        <span className="font-bold text-on-surface">{formatCurrencyFromCents(split.subtotalCents)}</span>
      </div>
      {split.discountCents > 0 && (
        <div className="flex justify-between items-center text-sm">
          <span className="text-on-surface-variant font-medium">{buildChargeLabel('Discount', discount)}</span>
          <span className="font-bold text-secondary">−{formatCurrencyFromCents(split.discountCents)}</span>
        </div>
      )}
      {(split.serviceChargeCents > 0 || serviceCharge.enabled) && (
        <div className="flex justify-between items-center text-sm">
          ...
          <span className="font-bold text-primary">+{formatCurrencyFromCents(split.serviceChargeCents)}</span>
        </div>
      )}
      {(split.gstCents > 0 || gst.enabled) && (
        <div className="flex justify-between items-center text-sm">
          <span className="text-on-surface-variant font-medium">{buildChargeLabel('GST / Tax', gst)}</span>
          <span className="font-bold text-primary">+{formatCurrencyFromCents(split.gstCents)}</span>
        </div>
      )}
      {activeChargesCents !== 0 && (
        <div className="flex justify-between items-center text-sm">
          ...
          <span className="font-bold text-primary">
            {activeChargesCents > 0 ? '+' : ''}{formatCurrencyFromCents(activeChargesCents)}
          </span>
        </div>
      )}
