...
function buildContextText(step: SimpleWizardStep, context: WizardProgressContext): string {
  ...
  const receiptPrefix =
    context.totalReceipts > 1
      ? ` · Receipt ${context.receiptNumber} of ${context.totalReceipts}`
      : '';
  ...
  switch (step) {
    ...
    case 'items': {
      const itemNumber =
        context.detectedItemsCount === 0
          ? 0
          : Math.min(context.activeItemIndex + 1, context.detectedItemsCount);
