...
describe('LegacyReceiptSplitterPage advanced mode integration', () => {
  ...
  it('re-sanitizes item assignments when people list changes', async () => {
    ...
    const bobOption = Array.from(assigneeSelect.options).find(
      (option) => option.textContent === 'Bob',
    );
  ...
  it('copies share text to clipboard when copy button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(navigator.clipboard.writeText).toHaveBeenCalledWith(expect.stringContaining('total:'));
    });
  });
...
describe('LegacyReceiptSplitterPage simple wizard integration', () => {
  ...
  it('defaults simple assignment to all selected and supports review/edit before final', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Ben', pressed: false })).toHaveAttribute(
      'aria-pressed',
      'false',
    );
  ...
  it('supports select all and select none shortcuts in simple item chooser', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Alice', pressed: false })).toHaveAttribute(
      'aria-pressed',
      'false',
    );
    expect(screen.getByRole('button', { name: 'Ben', pressed: false })).toHaveAttribute(
    ...
    expect(screen.getByRole('button', { name: 'Alice', pressed: true })).toHaveAttribute(
      'aria-pressed',
      'true',
    );
    expect(screen.getByRole('button', { name: 'Ben', pressed: true })).toHaveAttribute(
      'aria-pressed',
      'true',
    );
  });
