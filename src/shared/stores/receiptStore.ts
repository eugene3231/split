...
function createBlankReceipt(people: Person[], uxMode: 'simple' | 'advanced', name: string): Receipt {
  return {
    id: createId(),
    name,
...
function getScanState(scanStateByReceipt: Record<string, ReceiptScanState>, receiptId: string): ReceiptScanState {
...
export const useReceiptStore = create<ReceiptStore>((set, get) => {
    ...
    applyOcrPayload(
      ...
      (next) => {
        set((state) => ({
          receipts: state.receipts.map((r) =>
            r.id === receiptId
              ? { ...r, gst: resolveSetStateAction(r.gst, next) }
              : r,
          ),
        }))
      },
  ...
  setScanStatus: (receiptId, next) =>
    set((state) => {
      ...
      return { scanStateByReceipt: updateScanState(state.scanStateByReceipt, receiptId, { scanStatus: resolveSetStateAction(current.scanStatus, next) }) }
    }),
  setScanError: (receiptId, next) =>
    set((state) => {
      const current = getScanState(state.scanStateByReceipt, receiptId)
      return { scanStateByReceipt: updateScanState(state.scanStateByReceipt, receiptId, { scanError: resolveSetStateAction(current.scanError, next) }) }
    }),
  setScanWarnings: (receiptId, next) =>
    set((state) => {
      const current = getScanState(state.scanStateByReceipt, receiptId)
      return { scanStateByReceipt: updateScanState(state.scanStateByReceipt, receiptId, { scanWarnings: resolveSetStateAction(current.scanWarnings, next) }) }
    }),
    ...
    set((state) => {
      ...
      for (const [id, s] of Object.entries(next)) {
        if (s.isScanning) {
          ...
          next[id] = { ...s, loadingMessageIndex: nextIndex, loadingMessage: FUNNY_LOADING_MESSAGES[nextIndex] }
        }
      }
  ...
  handleScanReceipt: async () => {
    ...
    if (!geminiApiKeyInput.trim()) {
      set((state) => ({
        scanStateByReceipt: updateScanState(state.scanStateByReceipt, scanReceiptId, { scanError: 'Missing Gemini API key. Enter it above.' }),
      }))
      return
    }
    ...
    const setScanStatusForReceipt = (next: SetStateAction<string>) =>
      set((state) => {
        ...
        return { scanStateByReceipt: updateScanState(state.scanStateByReceipt, scanReceiptId, { scanStatus: resolveSetStateAction(current.scanStatus, next) }) }
      })
    const setScanWarningsForReceipt = (next: SetStateAction<string[]>) =>
      set((state) => {
        const current = getScanState(state.scanStateByReceipt, scanReceiptId)
        return { scanStateByReceipt: updateScanState(state.scanStateByReceipt, scanReceiptId, { scanWarnings: resolveSetStateAction(current.scanWarnings, next) }) }
      })
    ...
    try {
      ...
      set((state) => ({
        scanStateByReceipt: updateScanState(state.scanStateByReceipt, scanReceiptId, { scanError: message }),
      }))
    } finally {
      set((state) => ({
        scanStateByReceipt: updateScanState(state.scanStateByReceipt, scanReceiptId, {
