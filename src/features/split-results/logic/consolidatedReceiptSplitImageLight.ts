...
function drawGrandTotalCard(
  ...
  ctx.fillText(formatCurrencyFromCents(args.consolidatedSplit.grandTotalCents), args.x + CARD_PAD, args.y + 92)
...
function measurePersonCardHeight(personId: string, splitByReceipt: SplitResult[], receipts: Receipt[]): number {
  ...
  for (let i = 0; i < receipts.length; i++) {
    ...
    bodyH += RECEIPT_LABEL_H + lineItems.length * ITEM_ROW_H + DIVIDER_H + chargeRows * CHARGE_ROW_H + 16
  }
...
function drawPersonCard(
  ...
  for (let i = 0; i < args.receipts.length; i++) {
    ...
    drawLightTwoColumnRow(ctx, {
      x: nestedInnerX, y: rowY + 24, width: nestedInnerW,
    ...
    for (const line of lineItems) {
      ...
      drawLightTwoColumnRow(ctx, {
        x: nestedInnerX + 16, y: rowY + 22, width: nestedInnerW - 16,
        label: line.name,
