...
export async function generateReceiptSplitImage(
  options: GenerateReceiptSplitImageOptions,
): Promise<Blob> {
...
function drawSummaryCard(context: CanvasRenderingContext2D, args: SummaryCardArgs): number {
  ...
  if (args.split.discountCents > 0) {
    rows.push({
      label: buildChargeLabel('Whole-Bill Discount', args.discount),
      value: `−${formatCurrencyFromCents(args.split.discountCents)}`,
      valueColor: '#6ee7b7',
    });
  }
  ...
  rows.push(
    {
      ...
      value: formatCurrencyFromCents(args.split.serviceChargeCents),
    },
    {
      label: buildChargeLabel('GST / Tax', args.gst),
      value: formatCurrencyFromCents(args.split.gstCents),
    },
    {
      label: 'Total Assigned',
      value: formatCurrencyFromCents(args.split.grandTotalCents),
      emphasized: true,
    },
  );
  ...
  if (args.reconciliationCents !== null) {
    ...
    rows.push(
      ...
      {
        label: 'Difference',
        value: formatCurrencyFromCents(args.reconciliationCents),
        valueColor: args.reconciliationCents === 0 ? '#6ee7b7' : '#fcd34d',
      },
    );
  }
...
function measurePersonCardHeight(
  ...
  const perRow = includeItemDetails
    ? LINE_TO_DETAIL_H + LINE_ROW_DETAIL_H + ITEM_GAP
    : LINE_ROW_H + ITEM_GAP;
