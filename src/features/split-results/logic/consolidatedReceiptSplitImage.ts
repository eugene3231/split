...
function measurePersonCardHeight(
  ...
  for (let i = 0; i < receipts.length; i++) {
    ...
    bodyHeight +=
      RECEIPT_ROW_H +
      lineItems.length * ITEM_ROW_H +
      DIVIDER_H +
      chargeRows * CHARGE_ROW_H +
      RECEIPT_BOTTOM_GAP;
  }
...
function drawPersonCard(
  ...
  for (let i = 0; i < args.receipts.length; i++) {
    ...
    drawTwoColumnRow(context, {
      ...
      width: innerWidth,
      label: 'Items',
      value: formatCurrencyFromCents(receiptSplit?.subtotalByPersonCents[args.person.id] ?? 0),
      emphasized: false,
      valueColor: '#e2e8f0',
      size: 19,
    });
...
export async function generateConsolidatedSplitImage(
  options: GenerateConsolidatedSplitImageOptions,
): Promise<Blob> {
