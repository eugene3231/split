...
export function SplitTotalsCard({
  split,
  discount,
  serviceCharge,
  gst,
  ...
  const receiptTotalCents =
    reconciliationCents !== null ? reconciliationCents + split.grandTotalCents : null;
  ...
  return (
    <article className="overflow-hidden rounded-xl border border-white/8 bg-slate-900 shadow-lg shadow-black/20">
      <div className="border-b border-sky-500/50 bg-sky-500/15 px-4 py-3">
        <p className="text-sm font-bold text-slate-100">Total Assigned</p>
      ...
      <div className="space-y-2 p-4 text-sm">
        ...
        {split.discountCents > 0 ? (
          <SummaryRow
            label={discountLabel}
            value={`−${formatCurrencyFromCents(split.discountCents)}`}
            tone="ok"
          />
        ) : null}
        <SummaryRow
        ...
        {receiptTotalCents !== null ? (
          <>
            <SummaryRow label="Receipt Total" value={formatCurrencyFromCents(receiptTotalCents)} />
            <SummaryRow
              label="Difference"
            ...
            {reconciliationCents! < 0 && onApplyDiscount ? (
              ...
              >
                Apply {formatCurrencyFromCents(Math.abs(reconciliationCents!))} discount to
                reconcile
              </button>
            ) : null}
          </>
