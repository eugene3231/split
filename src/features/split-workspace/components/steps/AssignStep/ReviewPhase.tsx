...
type ReviewPhaseProps = {
  ...
  itemCount: number;
  onEditItem: (row: AssignmentReviewRow) => void;
};

type ReviewReceiptGroup = {
  receiptId: string;
  receiptName: string;
  rows: AssignmentReviewRow[];
};

function groupReviewRowsByReceipt(rows: AssignmentReviewRow[]): ReviewReceiptGroup[] {
  const groups: ReviewReceiptGroup[] = [];
  for (const row of rows) {
    const currentGroup = groups[groups.length - 1];
    if (currentGroup?.receiptId === row.receiptId) {
      currentGroup.rows.push(row);
    } else {
      groups.push({ receiptId: row.receiptId, receiptName: row.receiptName, rows: [row] });
    }
  }
  return groups;
}
...
export function ReviewPhase({ rows, itemCount, onEditItem }: ReviewPhaseProps) {
  const groups = groupReviewRowsByReceipt(rows);
  const showReceiptHeaders = groups.length > 1;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-6">
        {groups.map((group) => (
          <div key={group.receiptId} className="space-y-3">
            {showReceiptHeaders && (
              <h3 className="flex items-center gap-1.5 text-sm font-bold text-on-surface-variant">
                <span className="material-symbols-outlined text-sm">receipt_long</span>
                {group.receiptName || 'Untitled receipt'}
              </h3>
            )}
