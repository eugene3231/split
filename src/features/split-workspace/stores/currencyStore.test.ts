...
import * as exchangeRateApi from '@features/split-workspace/api/exchangeRateApi';
import * as exchangeRateStorage from '@features/split-workspace/logic/exchangeRateStorage';
...
describe('currencyStore', () => {
  ...
  it('fetchAndSetExchangeRates updates store when API returns rates', async () => {
    ...
    vi.spyOn(exchangeRateStorage, 'saveExchangeRates').mockImplementation(() => {});
    ...
    expect(exchangeRateStorage.saveExchangeRates).toHaveBeenCalledWith(mockRates);
  });
