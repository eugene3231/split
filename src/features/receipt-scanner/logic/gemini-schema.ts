...
const geminiChargeSchema = z.object({
  ...
  amount: z.number().nullable().describe('Absolute monetary amount of the charge, or null if not shown.'),
  percent: z.number().nullable().describe('Percentage rate of the charge (e.g. 9 for 9%), or null if not shown.'),
  confidence: z.number().nullable().describe('Confidence score between 0 and 1 for this detection.'),
  source: z.string().nullable().describe('Label or text on the receipt that triggered this detection, or null.'),
})
...
export const geminiReceiptSchema = z.object({
  ...
  subtotal: z.number().nullable().describe('Subtotal before taxes and charges, or null if not shown.'),
